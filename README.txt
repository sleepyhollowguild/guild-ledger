...
Guild member signup update:
- Customer kiosk now has a 🏮 Join the Guild button.
- New member signups write to Firebase at /guildMembers.
- Merchant dashboard has a Guild Members tab and Export Guild Members CSV button.

Firebase test rules should now include /guildMembers too:
{
  "rules": {
    "orders": { ".read": true, ".write": true },
    "sales": { ".read": true, ".write": true },
    "inventoryAdjustments": { ".read": true, ".write": true },
    "guildMembers": { ".read": true, ".write": true }
  }
}
